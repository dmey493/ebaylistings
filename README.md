...
pip install -e ".[dev]"
cp .env.example .env      # fill in ANTHROPIC_API_KEY, EBAY_CLIENT_ID/SECRET/RUNAME
...
| File | Role |
|---|---|
| `ebaylister/identify.py` | The two Claude calls. Pass 1 is vision → `IdentifiedItem`; pass 2 is text → `ListingDraft`. Both use structured outputs, so the result is a validated Pydantic object, never free text to parse. |
...
| `ebaylister/pipeline.py` | Orchestration and job state transitions. |
...
- **Costs.** Two Claude calls per item on `claude-opus-5` (a few cents each, mostly the photos). eBay API calls are free; eBay's normal final-value fees apply to sales.
...
- **Refusal fallback is on.** The Claude calls opt into Anthropic's server-side fallback so a safety-classifier decline on the primary model reruns on a fallback model instead of failing the job.
